  ...
  switch (!![]) {
    case /api\/sns\/v\d+\/note\/widgets/.test($request.url):
      try {
        ...
        (obj.data = {}), (body = JSON["stringify"](obj));
      } catch (s) {
    ...
    case /api\/sns\/v\d+\/homefeed\/categories\?/.test($request.url):
      try {
        let obj = JSON.parse(body);
        (obj.data.categories = obj.data.categories.filter((e) => !(e.oid == "homefeed.shop" || e.oid == "homefeed.sketch" || e.oid == "homefeed.live"))), (body = JSON.stringify(obj));
      } catch (s) {
